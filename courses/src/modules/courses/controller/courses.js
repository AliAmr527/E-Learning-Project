...
export const applyCourse = async (req, res) => {
	...
	const course = await courseModel.updateOne(
		...
		{ $addToSet: { requestedStudents: { _id: studentId, name: name } } },
		{ new: true }
	)
...
export const viewCurrentAndPastCourses = async (req, res) => {
	const currentCourses = await courseModel.find(
		{ enrolledStudents: { _id: req.body.id, name: req.body.name } },
		{ _id: 1, name: 1, duration: 1, category: 1, rating: 1 }
	)
	...
	const pastCourses = await courseModel.find(
		{ pastStudents: { _id: req.body.id, name: req.body.name } },
		{ _id: 1, name: 1, duration: 1, category: 1, rating: 1 }
	)
