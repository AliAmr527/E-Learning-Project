...
export const createCourse = async (req, res) => {
	...
	try {
		...
		if (response.data == "doesn't exist") return res.status(404).json({ message: "instructor not found" })

		const isExist = await courseModel.findOne({ name })
		if (isExist) return res.status(400).json({ message: "Course already exist" })
...
export const getCoursesForInstructor = async (req, res) => {
	...
	if (isExist.data == "doesn't exist") return res.status(404).json({ message: "instructor not found" })
...
export const applyCourse = async (req, res) => {
	...
	if (response.data == "doesn't exist") return res.status(404).json({ message: "student not found" })
...
export const cancelCourseEnrollment = async (req, res) => {
	...
	if (!course.modifiedCount) return res.status(404).json({ message: "you weren't enrolled in this course" })
...
export const acceptRequest = async (req, res) => {
	...
	if (!isExist) return res.status(404).json({ message: "you don't own this course" })
...
export const rejectRequest = async (req, res) => {
	...
	if (!course) return res.status(404).json({ message: "you don't own this course or this course doesn't exist" })
